 ...
 */
function links($interface)
{
    // Load people and roles from database:
    $links = new LinkList();
    $links->assign($interface);
    $linkTypes = new LinkTypeList();
    $linkTypes->assign($interface);
    
    // Display page with appropriate Javascript:
    $interface->addJavascript('edit_links.js');
    $interface->showPage('links.tpl');
}
?>